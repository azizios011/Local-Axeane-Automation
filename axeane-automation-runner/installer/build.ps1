<# 
.SYNOPSIS 
     Full build script for Axeane Automation MSI. 
     Run from repo root: .\axeane-automation-runner\installer\build.ps1 
  
 .REQUIREMENTS 
     - Rust + cargo (stable) 
     - Node.js + npm 
     - WiX Toolset v4 (dotnet tool install --global wix) 
     - Internet access for first run (downloads Python embeddable) 
 #> 
 ...
 Pop-Location 
 Log "Frontend build complete → $FrontendDir\out" 
  
 # ── Step 2: Build Rust binary ───────────────────────────────────────────────── 
 Log "Building Rust binary (release)..." 
 Push-Location $RunnerDir 
     cargo build --release 
     if (-not (Test-Path $ReleaseExe)) { Die "Rust build failed — exe not found at $ReleaseExe" } 
 Pop-Location 
 Log "Rust build complete → $ReleaseExe" 
 ...
 if (-not (Test-Path "$PythonDir\python.exe")) { 
     ...
     # The ._pth file disables site by default — we need to uncomment import site 
 ...
 # ── Step 4: Install backend dependencies into embedded Python ───────────────── 
 ...
 Log "MSI ready → $OutDir\Axeane-Automation-Setup.msi" 
 Log "" 
 Log "Build complete!" 
